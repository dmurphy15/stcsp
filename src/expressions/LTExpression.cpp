...
LTExpression::LTExpression(Expression &a, Expression &b) :
        Expression({a, b}, false),
        mExpr1(a),
        mExpr2(b) {}

int LTExpression::evaluate(SearchNode &context, int time) const
{
    return mExpr1.evaluate(context, time) < mExpr2.evaluate(context, time);
}

domain_t LTExpression::getDomain(SearchNode &context, int time) const
{
...
domain_t LTExpression::getInitialDomain() const
{
